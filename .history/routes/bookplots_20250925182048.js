...
router.get('/option/:plotId', requireLogin, async (req, res) => {
  ...
  try {
    ...
    // Render payment option page and pass bookingId
...
router.post('/create-checkout-session', requireLogin, async (req, res) => {
  ...
  try {
    ...
    const paymentType = amount >= plot.price ? 'fullpayment' : 'downpayment';
    await db.query(
      `INSERT INTO payment_tbl
       (booking_id, user_id, amount, method, status, transaction_id, created_at, payment_type)
       VALUES (?, ?, ?, 'card', 'pending', NULL, NOW(), ?)`,
      [booking_id, userId, amount, paymentType]
    );
...
router.get('/success', async (req, res) => {
  try {
    ...
    // 1️⃣ Insert payment record (paid)
    await db.query(
      `INSERT INTO payment_tbl
    ...
    // 2️⃣ Update booking status and plot availability
    await db.query(
