...
router.get('/', requireAdmin, async (req, res) => {
  try {
    // Join the latest payment row per booking (if any) so admin sees the most
    // recent method/status/type without duplicating bookings for each payment.
    const [rows] = await db.query(`
      ...
      LEFT JOIN (
        -- derived table selecting the latest payment per booking
