  ...
  try {
    ...
    if (paymentData) {
      ...
      const [bookingRows] = await db.query(
        `SELECT b.booking_id, b.user_id, CONCAT(u.firstName, ' ', u.lastName) AS user_name, u.email AS user_email, 
...
router.get("/installment-success", async (req, res) => {
  ...
  console.log('Installment-success: paymentData snapshot=', paymentData);
      ...
      try {
        const [bookingRows] = await db.query(
          `SELECT b.booking_id, b.user_id, CONCAT(u.firstName, ' ', u.lastName) AS user_name, u.email AS user_email, pm.plot_number, pm.location, pm.price
           FROM booking_tbl b
           JOIN user_tbl u ON b.user_id = u.user_id
           JOIN plot_map_tbl pm ON b.plot_id = pm.plot_id
           WHERE b.booking_id = ? LIMIT 1`,
        ...
        if (bookingRows.length) {
          ...
          const receiptHtml = await ejs.renderFile(path.join(__dirname, '../views/receipt.ejs'), { receipt: receiptData });
          const pdfBuffer = await generateReceiptPdf(receiptData);
          await transporter.sendMail({
            from: `"Everlasting Cemetery" <${process.env.EMAIL_USER}>`,
            to: receiptData.user_email,
