...
router.post('/', requireAdmin, async (req, res) => {
  try {
    ...
    // If admin recorded a cash payment, insert a payment row and update plot availability/user
    try {
      const paymentMethod = req.body.payment_method || 'none';
      const cashAmount = req.body.cash_amount ? parseFloat(req.body.cash_amount) : 0;
      if (paymentMethod === 'cash' && !Number.isNaN(cashAmount) && cashAmount > 0) {
        const tx = `CASH-${bookingId}-${Date.now()}`;
        const [payRes] = await db.query(
          `INSERT INTO payment_tbl (booking_id, user_id, plot_id, amount, method, transaction_id, status, paid_at, due_date, payment_type, months, monthly_amount, total_paid)
           VALUES (?, ?, ?, ?, ?, ?, 'paid', NOW(), NULL, 'fullpayment', NULL, NULL, ?)`,
          [bookingId, userId, plotId, cashAmount, 'cash', tx, cashAmount]
        );
