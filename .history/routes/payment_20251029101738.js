...
// --- SMTP verification endpoint (dev/admin only) ---
// GET /admin/smtp-verify?testEmail=you@example.com
router.get('/admin/smtp-verify', async (req, res) => {
  try {
    // Restrict in production to admin users
    ...
    const testEmail = req.query.testEmail || process.env.TEST_EMAIL;
    ...
    if (testEmail) {
      ...
      await transporter.sendMail({
        from: `"Everlasting Cemetery" <${process.env.EMAIL_USER}>`,
        to: testEmail,
        subject: 'SMTP Verification Test - Everlasting Cemetery',
        text: 'This is a test email to verify SMTP configuration for Everlasting Cemetery.'
      });
      result.sent = true;
