  ...
  console.log('Installment-success: paymentData snapshot=', paymentData);
      ...
      try {
        ...
        if (bookingRows.length) {
          ...
          // Use helper to send receipt email for installment payments
          try {
            const mailResult = await sendReceiptEmail(receiptData, booking);
            if (!mailResult.ok) console.error('Failed to send installment receipt email', mailResult.error);
          } catch (mailErr) {
            console.error('Unexpected error sending installment receipt email:', mailErr);
          }
        }
      } catch (mailErr) {
